...
(defpackage :allegretto-prolog-1
  (:use :cl)
  (:shadow #:symbol)
  (:export #:?-
           #:<-
...
(in-package :allegretto-prolog-1)
...
(defun symbol (&rest args)
  ...
  (with-standard-io-syntax
    (intern (format nil "~:@(~{~A~}~)" args) "ALLEGRETTO-PROLOG-1")))
...
(defvar *trail* (cons 0 (make-array 512 :element-type 'list :initial-element nil)))
...
(defun grow-prolog-trail (trail)
  (let* ((size (length (trail-vec trail)))
         (new-trail-vec (make-array (* 2 size) :element-type 'list :initial-element nil)))
...
(defun make-functor-name (symbol arity)
  "Return the symbol: symbol/arity"
  (make-symbol (concatenate 'string (string symbol) "/" (princ-to-string arity))))

(defun ensure-functor-locf* (symbol arity)
  (let* ((functor (make-symbol (concatenate 'string (string symbol) "/" (princ-to-string arity))))
         (prop (cons arity functor)))
    (pushnew prop (get symbol 'functor) :key #'car)
    prop))
...
(progn
  ...
  (defun let-de-1 (setter cont)
    (declare (optimize (speed 3) (safety 0) (debug 0)))
    (funcall setter (cons var-tag unbound))
    (funcall cont))
