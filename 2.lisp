...
(BLOCK PROLOG
  (FLET ((PROLOG-CLAUSES (TRAIL CONT)
           (DECLARE (IGNORABLE TRAIL CONT))
           (LET-DE ((?X (?)))
                   (DE-CONSIFY (MAKE-PREDICATE 'FIB '2) TRAIL CONT '22 ?X))))
    (DECLARE (DYNAMIC-EXTENT #'PROLOG-CLAUSES))
    (LET ((TRAIL (EXCL::FAST *TRAIL*)))
      (IF TRAIL
          (PROLOG-CLAUSES TRAIL #'IGNORER)
        (EXCL::WITH-RESOURCE (TRAIL TRAIL)
          (SETF (TRAIL-NDX TRAIL) 0)
          (LET ((*TRAIL* TRAIL)) (PROLOG-CLAUSES TRAIL #'IGNORER))))))
  NIL)

;;(de-consify $predicate $trail $cont $args...)


(APPLY #'INTERPRETED-DE-WRAPPER
       (LAMBDA (CONSES) ($PREDICATE $TRAIL $CONT $ARGS...))
       (MAKE-LIST 0))


(INTERPRETED-DE-WRAPPER #'values (list 1 2 3))
...
(LET (?X)
  (FLET ((STUFFIT (NEW) (SETF ?X NEW)))
    (LET-DE-1 #'STUFFIT (LAMBDA NIL (LET-DE NIL)))))


(LET-DE-1 #'STUFFIT (LAMBDA NIL (LET-DE NIL)))
(LET-DE-1 setter cont)

;; disassembly of #<Function LET-DE-1>
;; formals: SETTER CONT
;; constant vector:
0: "PrologVar"
1: "Unbound"

;; code start: #x10003f50f90:
   0: 48 81 ec a8 00 subq	%RSP,$168       ; 21
      00 00 
   7: 4c 89 74 24 08 movq	[%RSP+8],%R14
  12: 4c 89 7c 24 60 movq	[%RSP+96],%R15
  17: 48 89 b4 24 98 movq	[%RSP+152],%RSI ; CONT
      00 00 00 
  25: 4d 8b 6e 36    movq	%R13,[%R14+54]  ; "PrologVar"
  29: 4d 8b 66 3e    movq	%R12,[%R14+62]  ; "Unbound"
  33: 48 8d 84 24 81 leaq	%RAX,[%RSP+129]
      00 00 00 
  41: 48 c7 40 e7 10 movq	[%RAX-25],$16   ; 2
      00 00 00 
  49: 4c 89 78 ff    movq	[%RAX-1],%R15
  53: 4c 89 60 f7    movq	[%RAX-9],%R12
  57: 4c 89 68 ef    movq	[%RAX-17],%R13
  61: 48 c7 40 df 70 movq	[%RAX-33],$112  ; 14
      00 00 00 
  69: 49 89 c5       movq	%R13,%RAX
  72: 48 89 bc 24 90 movq	[%RSP+144],%RDI ; SETTER
      00 00 00 
  80: 4c 89 ef       movq	%RDI,%R13
  83: 4c 8b b4 24 90 movq	%R14,[%RSP+144] ; SETTER
      00 00 00 
  91: 48 c7 c0 08 00 movq	%RAX,$8 ; 1
      00 00 
  98: 41 ff 97 9f 05 call	*[%R15+1439]    ; SYS::FUNCALL-CHECK
      00 00 
 105: 4c 8b b4 24 98 movq	%R14,[%RSP+152] ; CONT
      00 00 00 
 113: 31 c0          xorl	%EAX,%EAX
 115: 41 ff 97 9f 05 call	*[%R15+1439]    ; SYS::FUNCALL-CHECK
      00 00 
 122: 4c 8b b4 24 b8 movq	%R14,[%RSP+184]
      00 00 00 
 130: 4c 89 bc 24 c8 movq	[%RSP+200],%R15
      00 00 00 
 138: 48 8d a4 24 a8 leaq	%RSP,[%RSP+168]
      00 00 00 
 146: c3             ret
 147: 90             nop
...
(LET (?X)
  (FLET ((STUFFIT (NEW) (SETF ?X NEW)))
    (LET-DE-1 #'STUFFIT (LAMBDA NIL (LET-DE ((?Y (?))) 3)))))
...
(LET (?X)
  (FLET ((STUFFIT (NEW) (SETQ ?X NEW)))
    (MY-LET-DE1 #'STUFFIT (LAMBDA NIL (MY-LET-DE ((?Y (?))) 3)))))
